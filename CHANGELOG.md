...
Entries from 2026-08-06 onward follow [Keep a Changelog 1.1.0](https://keepachangelog.com/en/1.1.0/).
Entries below that point were generated by [standard-version](https://github.com/conventional-changelog/standard-version)
and inherited from the upstream project; they are kept as-is rather than rewritten.

## [Unreleased]

### Changed

- `eng-tools` is no longer a GitHub fork of [`CorentinTh/it-tools`](https://github.com/CorentinTh/it-tools).
  The repository has left the upstream fork network and is maintained as an independent project.
  It remains a derivative work under the GPL-3.0 license; attribution to the original project is
  retained in `README.md`, `LICENSE`, the About page, `public/humans.txt`, and the application footer.
- The product name shown in the interface, page metadata, issue templates, and `package.json` is now
  "Engineering Tools" (`eng-tools`) rather than "IT-Tools". The package name moved from `it-tools` to
  `eng-tools`.
- `package.json` — the `license` field was corrected from the non-SPDX string `GNU GPLv3` to
  `GPL-3.0-only`, and Corentin Thomasset moved from `author` to `contributors`, credited as the
  original author of the project this one is derived from.
- The application footer now reads "Derived from it-tools" rather than "Forked from it-tools".

### Fixed

- `index.html` — the `<link rel="canonical">` tag and the Open Graph `og:url` and `og:image` tags
  pointed at `https://it-tools.tech`, which told search engines this site was a duplicate of the
  upstream project. They now point at `https://eng-tools.kevininscoe.com`.

### Removed

- The X (Twitter) account button in the navbar and on the home-page banner. Both linked to
  `https://x.com/ittoolsdottech`, the upstream project's account, while one was labelled
  "Eng Tools X account". The `twitter:site` and `twitter:creator` meta tags naming upstream accounts
  were removed from `index.html` for the same reason.
- The About page text in the eight non-English locale files. It described the upstream project, its
  author's hosting costs, and a Vercel deployment that does not apply here. Those locales now fall
  back to the English About page.
